...
- founder intent from the August user stories
- original hand-sketched screen designs
- the locked corrections addendum
- earlier AI rewrite documents that are useful reference but not authoritative
...
3. `📄 TimeOut System Spec v1.1 — Corrections & Clarifications (Final Lock Addendum)`
4. `TimeOut Flow.pptx`
...
- The August stories carry the strongest founder/product intent.
- The locked addendum resolves conflicts and defines the current V1 scope.
- Parts 1, 2, and 3 are older rewrite/reference documents and may contain obsolete rules.
...
- `Emergency Daycare Pickup` is a special-case preset with additional education and additional information capture such as daycare location.
...
- Invitees should inherit the inviter's group identity.
- If a user belongs to more than one group, the product must keep group identity clear and avoid confusion between groups.
...
- The product should support the edge case of belonging to more than one group, even if multi-group UX polish is deferred from V1.
- The product must avoid:
  - accidentally creating a new local group when the user intended to invite
  - accidentally treating a share as a local invite

### AutoPing

- Sequential AutoPing is the default.
- Broadcast is limited to:
  - emergency daycare pickup
  - playdate
  - gathering RSVP
- Remove generic `batch mode` language from product/spec text.
- No 2-hour AutoPing cutoff in V1.
- AutoPing continues until:
  - first YES
  - requester cancels
  - list is exhausted
...
- Estimated duration exists mainly to help potential sitters evaluate whether they are available.
- Estimated duration is not binding on the parties.
- Actual details can be discussed off-app.
- Meaningful changes to a sit require cancel/restart.
- Errors can be corrected through:
  - a custom single-target request in some practical cases
  - transfer points
...
- A non-app responder may coordinate a sit, but no points are exchanged for that response.
- Family members who want to earn points for sitting other members must be actual subscribed members.
- A member-family edge case may use transfer points later, for example gifting points to a daughter/member.

### Reminders

- Reminder timing is:
  - 24 hours before sit start
  - 2 hours before sit start
- Reminder notifications must include a cancel option.
- Both requester and sitter may cancel before points are posted.
- Reminder and confirmation notifications should support direct action without requiring unnecessary navigation.

## Invite / Startup Path

This area is intentionally provisional and must be revalidated in a future session.

Current direction:

- The first invite text should come from a friend.
- The app may help the user select contacts and generate the invite message/link.
- The message should feel like:
  - `this looks good`
  - `I just went through the link and education cards`
  - `just click the link`
...
Open for next-session validation:
...
- This is the primary marketing surface for the Play Store.
- It should feel branded, warm, personal, and emotionally compelling.
- It should emphasize quick-ping presets and the value proposition.
- It should not look like a generic menu screen.

### Sit Request Screen

- This should feel compact, practical, and form-board-like.
- The time and duration matrix are key interaction elements.
- The design should preserve the sketch structure, not flatten it into generic cards.
- The sit start-time matrix is a `3 x 6` matrix.
- The sit duration matrix is a `3 x 6` matrix.

## Corrections To Older Rewrite Docs

Treat the following statements in Parts 1, 2, and 3 as outdated unless reapproved:
...
- any requirement that contradicts the fixed-preset or one-active-ping rules
...
Do not treat these as unresolved requirements right now, but keep them visible for the next product session:
